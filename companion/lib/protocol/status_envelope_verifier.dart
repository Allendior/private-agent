...
/// Verifies an unexpired, status-only proof without executing any phone action.
class StatusEnvelopeVerifier {
  static StatusEnvelopeResult verify({
    ...
    if (payload is! Map || signature is! String || sharedKey.length < 32) {
  ...
  static Object? _canonicalize(Object? value) {
    if (value is Map) {
      ...
      for (final entry in value.entries) {
        if (entry.key is! String) throw ArgumentError('protocol keys must be strings');
